...
static volatile int64_t SysTickCountMs = 0;
static volatile uint32_t sysDummy = 0;


static uint32_t previous_count = 0;

// Get the 64 bit timer counter value.
int64_t sysGetSysTimeMs()
{
	const int32_t current_count = HAL_GetTick();
	const int32_t diff_count = current_count - previous_count;
	SysTickCountMs += diff_count;

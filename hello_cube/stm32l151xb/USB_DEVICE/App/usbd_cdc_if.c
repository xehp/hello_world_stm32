...
// Uncomment if needed.
/*static inline int fifo_get_free_space(volatile struct Fifo *fifoPtr)
{
...
static uint8_t lcBuffer[7]; // Line coding buffer
...
static struct Fifo usbRxFifo;
static struct Fifo usbTxFifo;
...
/**
  ...
  * "If I understand correctly the CDC_Receive_FS is the callback function..."
  */
...
// User may call this to receive one char to USB virtual serial port.
// Returns -1 if there was nothing.
int cdcTakeChar()
{
	return (fifoIsEmpty(&usbRxFifo) ? -1 : fifoTake(&usbRxFifo));
}

// User may call this to send one char to USB virtual serial port.
// It will overwrite existing data if to much is written.
void cdcPutChar(int ch)
{
	fifoPut(&usbTxFifo, ch);
}

// This needs to be called periodically.
void cdcProcess(int ch)
{
	static uint8_t transmitBuf[60];
	static int transmitBufSize=0;

	// Try to send the data in FIFO
	while((transmitBufSize < sizeof(transmitBuf)) && (!fifoIsEmpty(&usbTxFifo)))
	{
		transmitBuf[transmitBufSize]=fifoTake(&usbTxFifo);
		transmitBufSize++;
	}

	if (transmitBufSize > 0)
	{
		const uint8_t r = CDC_Transmit_FS(transmitBuf, transmitBufSize);
		switch(r)
		{
			case USBD_OK:
				transmitBufSize = 0;
